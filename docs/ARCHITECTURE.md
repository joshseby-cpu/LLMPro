...
| File | Responsibility |
|---|---|
| [`ModelsBrowserView.swift`](../LLMPro/Features/Models/ModelsBrowserView.swift) | Three-section list: Downloading / HuggingFace results / Local models. Search bar with `mlx-community only` toggle. Per-local-row: trash + ✨ modify icon (greyed when the model is in active training). **A `model.isDiffusion` row shows a "Diffusion · chat only" badge** (a small inline `Label`, sparkles icon) flagging that the model works in Try-it-out but can't be fine-tuned. (The badge text is now slightly stale — DiffusionGemma also works in the **Code** tab, where tool-use is experimental; it's excluded only from Teach/Practice/DPO, since mlx-lm can't LoRA-train a diffusion LM.) Total-disk header. Confirmation alerts for delete (with bytes-freed message). Sheet for modify. |
