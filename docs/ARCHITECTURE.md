...
| [`ImageGenService.swift`](../LLMPro/Services/ImageGenService.swift) | `@MainActor @Observable` singleton (Story illustrations). `installed()`/`install()` gate the optional `mflux` add-on (delegates to `PythonRuntime`). `generate([Request])` writes a batch JSONL and runs [`generate_image.py`](../LLMPro/Resources/helpers/generate_image.py) **once** (FLUX loads once, reused for the whole chapter) via `ProcessRunner.spawn` inside a `withTaskCancellationHandler` (Stop terminates the subprocess); it consumes the JSON-event stdout on the main actor and `progress` (done/total + `loadingModel`) drives the UI. A busy-guard rejects an overlapping batch rather than loading a second FLUX. **`defaultModel` is the *ungated* pre-quantized mirror `dhairyashil/FLUX.1-schnell-mflux-4bit`** (the canonical `black-forest-labs/FLUX.1-schnell` is `gated: auto` → 401 without a token; the mirror needs none, ~9.6 GB) with `defaultBaseModel = "schnell"`; passes `HF_TOKEN` (Keychain) when present as a fallback for gated models. See [CONTRACTS §3 `generate_image.py`](CONTRACTS.md). |
...
| File | Responsibility |
|---|---|
| [`ImagineView.swift`](../LLMPro/Features/Imagine/ImagineView.swift) | The **Imagine** tab: free-form local text-to-image. A prompt bar (prompt + a **model picker** grouped **On your Mac** vs **Download & use** — the built-in `ImageModel.presets` (FLUX.1 schnell/dev × 4-/8-bit) plus any FLUX model already in the HF cache via `ImageGenService.downloadedNonPresetImageModels()` (diffusers-layout + FLUX-family filter, so chat LLMs and non-FLUX image models like SDXL are excluded) + a **size menu** (aspect Square/Landscape/Portrait/Wide/Tall × resolution Small/Standard/Large → dims rounded to /16) + count 1–4 + Generate/Stop + progress line) over a `LazyVGrid` gallery of results (`GalleryThumb`, async NSImage load). Builds N `ImageGenService.Request`s (random seeds) and calls `ImageGenService.generate(model:baseModel:steps:width:height:)` — the **same FLUX engine as Story illustrations**, so it works regardless of the chat model (dev uses ~20 steps, schnell 4). Model + aspect + resolution persist via `@AppStorage`. Cancellation-aware (Stop kills FLUX + deletes the partial); an install gate offers the one-time `mflux` install. Per-image: tap → `ImagePreviewSheet` (full-size + dims/seed + Save/Copy prompt/Delete); context menu adds "Use this prompt" / "Make another like this". |
