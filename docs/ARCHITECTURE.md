...
| [`LLMPro/Core/Theme.swift`](../LLMPro/Core/Theme.swift) | The app's small **design system**. `Color.brand` = the `AccentColor` asset (brand violet ≈ #6B4AFF light / #8C78FF dark, wired app-wide via `ASSETCATALOG_COMPILER_GLOBAL_ACCENT_COLOR_NAME: AccentColor` in `project.yml`, so every system control tints brand instead of system blue) + a root `.tint(.brand)` belt-and-suspenders. `.card(padding:cornerRadius:)` = the shared content-card surface (continuous-radius `RoundedRectangle` + `Color.primary.opacity(0.04)` fill + hairline `0.08` border + soft shadow, light/dark adaptive) — the canonical replacement for ad-hoc `.thinMaterial` cards. Also `sectionHeader(_:systemImage:)` and `Theme.brandGradient`. **Use these instead of re-rolling card chrome / hardcoding `.blue`.** |
...
| [`LLMPro/Services/GGUFImportService.swift`](../LLMPro/Services/GGUFImportService.swift) | `@MainActor @Observable`. Powers the Models-tab "Import GGUF" sheet. `precheck(path:)` reads a GGUF's arch+quant (via `gguf_to_mlx.py precheck`) to report whether the lightweight converter can handle it (F16/Q4_0/Q4_1/Q8_0 yes; K-quants no). `convert(path:outputName:)` runs `gguf_to_mlx.py convert` → new `models/<name>/` MLX model, gated on the precheck. `downloadFromHuggingFace(repo:filename:)` pulls a single .gguf. **One-tap combo:** `downloadAndConvert(repo:)` (used by the search-result "Download & convert" button) calls `bestConvertibleFile(repo:)` (prefers Q8_0 → plain Q4_0 → F16; throws if the repo only has k-quants/i-quants), downloads just that one file, then `convert`s it — so a GGUF LLM becomes a usable MLX model without pulling the whole multi-quant repo. **Optimize for MLX**: for a full-precision (F16/F32) GGUF, `convert(...optimize:optimizeBits:)` runs an `mlx_lm convert -q` pass (quantizeInPlace → temp dir → swap) for a ~4× smaller canonical-MLX model; already-quantized GGUFs are left as-is (re-quantizing degrades them). No PyTorch — uses mlx.core's native GGUF loader. **Chat-template fallback:** when the source GGUF carries no `tokenizer.ggml.chat_template` metadata, `gguf_to_mlx.py` now writes a **per-architecture default** chat template (ChatML for qwen2/qwen2moe/qwen3, Gemma turn format, Llama-3 headers, Phi-3, Mistral) so converted INSTRUCT models chat out of the box; the `done` event reports `chat_template_source` (`metadata` \| `fallback-<arch>` \| `none`). See [`CONTRACTS.md`](CONTRACTS.md#gguf_to_mlxpy--gguf--mlx-import-chat-template-fallback). |
| [`LLMPro/Services/FuseService.swift`](../LLMPro/Services/FuseService.swift) | `actor`. The GGUF/Ollama export engine. `fuse(dequantize:)` merges a LoRA (with `--dequantize` for GGUF so a quantized MLX base becomes an HF checkpoint the converter can read). `convertDirToGGUF(modelDir:ggufPath:quant:)` is the shared converter: base types (f16/bf16/q8_0) go straight through `convert_hf_to_gguf.py`; **k-quants** (`GGUFQuant` Q4_K_M/Q5_K_M/Q6_K) convert to f16 then run the compiled `llama-quantize`. `verifyGGUF()` is the post-export **self-test** — runs `llama-completion` and only passes if the GGUF emits coherent UTF-8 (no U+FFFD). `convertModelToGGUF()` (per-model, no adapter) and `fuseAndConvertExternalGGUF()` (adapter) both run convert→quantize→self-test and return a `GGUFSelfTest`. `ggufRoundTripWarning()` hard-blocks hybrid `qwen3_5` (the MLX→GGUF norm double-shift). `llamaToolsInstalled()` gates k-quants/self-test on the compiled binaries (`llamaQuantizeBin`/`llamaCompletionBin` under `runtime/llama.cpp/build/bin/`). `installInOllama()` runs `ollama create`. `OllamaChatTemplate` has templates for Qwen / DeepSeek / Llama 3 / Phi / Mistral / raw. |
...
### Productivity & UX (2026-06-23 feature batch)

| File | Responsibility |
|---|---|
| [`LLMPro/Services/NotificationService.swift`](../LLMPro/Services/NotificationService.swift) | `@MainActor @Observable`. Local completion notifications (UserNotifications). `primeAuthorization()` lazily requests permission at a job's start; `trainingFinished/practiceFinished/exportFinished(name:success:)` post friendly banners. No-ops if denied. Hooked from `JobRegistry.markCompleted/markFailed`, `SelfImproveService`, and the export views. |
| [`LLMPro/Services/KeepAwakeService.swift`](../LLMPro/Services/KeepAwakeService.swift) | `@MainActor @Observable`. Holds `caffeinate -imsu` (via `ProcessRunner.spawn`) while ≥1 job runs. `refresh(runningCount:)` is called from `JobRegistry` transitions; gated by the `keepAwakeWhileTraining` pref (Settings → Runtime → Power). |
| [`LLMPro/Services/StorageService.swift`](../LLMPro/Services/StorageService.swift) | `@MainActor @Observable`. `scan()` measures every PathResolver dir off-main → per-category `Category` (bytes, item count, clearable, hint) + free space. `clear(_:)` wipes a clearable (regenerable) category's contents. Powers `StorageSettingsView`. |
| [`LLMPro/Features/Settings/StorageSettingsView.swift`](../LLMPro/Features/Settings/StorageSettingsView.swift) | The "Storage" tab in Settings — per-category breakdown, proportional bar, reveal/clear, rescan. |
| [`LLMPro/Features/MenuBar/JobStatusMenuBar.swift`](../LLMPro/Features/MenuBar/JobStatusMenuBar.swift) | `JobStatusMenuBarContent` — the menu-bar panel shown via a `MenuBarExtra` scene in `LLMProApp` while a job runs. Reuses `TrainingNarrator` for phase/ETA/stars; Open-Progress + Stop buttons. |
| [`LLMPro/Services/DatasetInsightsService.swift`](../LLMPro/Services/DatasetInsightsService.swift) | Pure `[ChatRow]` analysis: row/message/est-token counts, role balance, length histogram, duplicate + empty-message counts. |
| [`LLMPro/Services/DatasetLinter.swift`](../LLMPro/Services/DatasetLinter.swift) | `lint(_:) -> [Issue]` (missing user/assistant turn, empty content, dups, over-long, system-not-first) + non-destructive `cleaned(_:)`. |
| [`LLMPro/Features/Datasets/DatasetInsightsView.swift`](../LLMPro/Features/Datasets/DatasetInsightsView.swift) · [`DatasetLintSheet.swift`](../LLMPro/Features/Datasets/DatasetLintSheet.swift) | Insights disclosure + "Check" linter sheet, surfaced from `DatasetDetailView` (operate on the loaded split's rows; clean → sets rows + dirty). |
| [`LLMPro/Features/Chat/ConversationMarkdownExporter.swift`](../LLMPro/Features/Chat/ConversationMarkdownExporter.swift) | `@MainActor`. Renders one/both arena sessions to Markdown + saves via NSSavePanel. "Export chat…" in ArenaView. |
| [`LLMPro/Features/Chat/SystemPromptPresetStore.swift`](../LLMPro/Features/Chat/SystemPromptPresetStore.swift) | `@MainActor @Observable`. 6 built-in personas + custom presets persisted to `system_prompt_presets.json` (no SwiftData). Drives the Persona menu in ArenaView; the same row exposes the now-active top-p/seed sampling controls. |

### Productivity & UX round 2 (2026-06-23)

| File | Responsibility |
|---|---|
| [`LLMPro/Services/FavoritesStore.swift`](../LLMPro/Services/FavoritesStore.swift) | `@MainActor @Observable`. Pins for models (`DetectedModel.id`) + datasets (`DatasetRecord.id.uuidString`), JSON under app-support. Float pinned items to the top in `ModelsBrowserView` / `DatasetsView` (star button + pinned-first sort). |
| [`LLMPro/Services/ModelMetaStore.swift`](../LLMPro/Services/ModelMetaStore.swift) · [`Features/Models/ModelNotesSheet.swift`](../LLMPro/Features/Models/ModelNotesSheet.swift) · [`Features/Models/RenameModelSheet.swift`](../LLMPro/Features/Models/RenameModelSheet.swift) | User notes + tags + **display-name alias (rename)** per model (JSON keyed by model id, no schema change). Tag chips render in the model row; "Notes & tags…" opens the notes sheet; **"Rename…"** (context menu on both LLM and image rows) opens `RenameModelSheet` → `ModelMetaStore.rename(_:for:default:)`. The alias is applied wherever the name shows (Models-tab rows + Imagine picker) via `displayName(for:default:)`; it's an alias only — the on-disk folder/repoID are untouched, so loading/training keep working. Reset-to-original clears it. |
| [`LLMPro/Services/DockProgressService.swift`](../LLMPro/Services/DockProgressService.swift) | `@MainActor enum`. Sets `NSApp.dockTile.badgeLabel` to the current "n/m" step from `JobRegistry` transitions (refresh on attach/recordStep/markCompleted/markFailed/stop); nil when idle. |
| [`LLMPro/Services/TrainingRunReport.swift`](../LLMPro/Services/TrainingRunReport.swift) · [`Features/Monitor/TrainingRunReportView.swift`](../LLMPro/Features/Monitor/TrainingRunReportView.swift) | `@MainActor`. Render a `TrainingJob` to a Markdown report (facts + hyperparam YAML + downsampled loss table + stars) → NSSavePanel; the view is the in-app preview. "View/Export report…" in Past lessons. |
| [`LLMPro/Features/Monitor/TrainingComparisonView.swift`](../LLMPro/Features/Monitor/TrainingComparisonView.swift) | Swift-Charts overlay of train-loss curves across selected past runs (multi-select list + chart). "Compare" toolbar button in `TrainingHistoryView`. |
| [`LLMPro/Core/CodeBlockParser.swift`](../LLMPro/Core/CodeBlockParser.swift) · [`Features/Chat/MessageContentView.swift`](../LLMPro/Features/Chat/MessageContentView.swift) | Fenced-code splitter + a chat-message renderer with per-code-block Copy buttons; replaces the plain `Text` in `ChatView.MessageBubble` (which also gained a "Try again"/regenerate + Copy row). `MessageContentView` strips reasoning (below) before splitting code. |
| [`LLMPro/Core/ReasoningStripper.swift`](../LLMPro/Core/ReasoningStripper.swift) | `visible(_:streaming:)` hides reasoning-model `<think>…</think>` output (complete pairs; a lone trailing `</think>` when the template ate the opener; and, when `streaming`, an unclosed opener). Used by `MessageContentView` (live display) AND to strip stored text on completion in `ChatSession` + `StoryGenerator` (so summaries / rolling context / exports stay clean). The Inspect CoT tab is a separate reasoning channel and is unaffected. |
| [`LLMPro/Core/ModelFit.swift`](../LLMPro/Core/ModelFit.swift) | Pre-flight RAM guard so an oversized model fails fast with a clear message instead of the OS OOM-killing it (the cryptic **"exited with code 9"** = SIGKILL). `tooLargeError(localModelPath:)` sums the model dir's `*.safetensors` bytes (resolving HF-cache symlinks) and returns a user-facing message when weights exceed **85% of physical RAM** (Apple-Silicon MLX unified memory → resident ≈ weight size; a 4-bit model reports its compressed size, so the same threshold works for fp16 and quantized). `exitMessage(code:tool:)` translates a SIGKILL exit (9 / 137) into a plain "ran out of memory" message. Called by `InferenceService.stream` and `MLXServerService.start` **before** spawning, and in their exit handlers. Verified live: a 240 GB fp16 120B on a 128 GB Mac is refused instantly instead of loading for ~30 s then crashing. |
| [`LLMPro/Features/Chat/PromptLibraryStore.swift`](../LLMPro/Features/Chat/PromptLibraryStore.swift) | `@MainActor @Observable`. 8 built-in coding test prompts + custom (JSON). Drives the "Prompts" menu in ArenaView's sampling row; `ChatSession.regenerateLast()` powers the regenerate button. |

### Productivity & UX round 3 (2026-07-01)

| File | Responsibility |
|---|---|
| [`LLMPro/Features/Dashboard/GettingStartedChecklist.swift`](../LLMPro/Features/Dashboard/GettingStartedChecklist.swift) | The Home 4-step loop checklist (replaces the single next-step card): live done states from models/datasets/finished jobs + the `onboarding.triedChat` flag (set by ArenaView's first send); all-done → dismissible 🎉 banner. |
| [`LLMPro/Features/Dashboard/RecentActivityFeed.swift`](../LLMPro/Features/Dashboard/RecentActivityFeed.swift) | Unified time-sorted Home feed over TrainingJob + DatasetRecord + EvalRun + SelfImproveRun (emoji + friendly line + relative time + deep link); replaces `recentJobs`, keeps rename via closure. |
| [`LLMPro/Features/Dashboard/DashboardQuickActions.swift`](../LLMPro/Features/Dashboard/DashboardQuickActions.swift) | Four prerequisite-aware launcher tiles (Teach / Try it out / Score / Practice) + a rotating 15-entry "tip of the day" card (`dashboard.tipOffset`). |
| [`LLMPro/Features/Models/ModelCardView.swift`](../LLMPro/Features/Models/ModelCardView.swift) | "About this model…" — `ModelFacts` (config.json facts + ~param count from `SafetensorsHeader.enumerateModel` off-main) + lineage (jobs from this base) + latest report card + notes/tags + GGUF-ready badge. |
| [`LLMPro/Features/Models/ModelCompareView.swift`](../LLMPro/Features/Models/ModelCompareView.swift) | Side-by-side two-model comparison (facts grid, differing rows brand-highlighted, GGUF/score rows). "Compare…" in the Local models header. |
| [`LLMPro/Services/ModelCardBuilder.swift`](../LLMPro/Services/ModelCardBuilder.swift) | `@MainActor enum`. Markdown builders: `modelCard` (HF-style card with training + eval facts) and `cloudREADME` (vLLM/TGI serve instructions for the cloud export). |
| [`LLMPro/Features/Export/ModelCardPreviewView.swift`](../LLMPro/Features/Export/ModelCardPreviewView.swift) | "Model card…" in Save & Use — assembles job/dataset/eval automatically, previews the markdown, Copy/Save. |
| [`LLMPro/Features/Eval/EvalLeaderboardView.swift`](../LLMPro/Features/Eval/EvalLeaderboardView.swift) | "Report cards" (trophy toolbar in Save & Use): score-over-time chart, best-per-(base+adapter) leaderboard, full run list. |
| [`LLMPro/Services/TrainingPresetStore.swift`](../LLMPro/Services/TrainingPresetStore.swift) | Saved training recipes (`training_presets.json`). `applying(_:to:)` merges recipe knobs while keeping per-run model/data/adapter. Preset bar lives inside Teach's Advanced disclosure. |
| [`LLMPro/Features/Shared/CommandPaletteView.swift`](../LLMPro/Features/Shared/CommandPaletteView.swift) | ⌘K palette (menu item via `.commands` in LLMProApp → `.openCommandPalette` → sheet in RootView). Prefix-then-contains ranking over all 13 tabs. |

The Save & Use tab also gained a fourth export target — **Host to the cloud**
(`ExportTarget.cloud`): `fuse --dequantize` → full-precision HF safetensors +
README with serve commands. Arch-agnostic (works for hybrid archs GGUF can't run).
...
| File | Responsibility |
|---|---|
| [`ModelsBrowserView.swift`](../LLMPro/Features/Models/ModelsBrowserView.swift) | Three-section card layout (**`ScrollView` + `LazyVStack`**, NOT a `List` — `List` renders a few dynamically-loaded search rows at zero height until scrolled): Downloading / HuggingFace results / Local models. Search bar with `mlx-community only` toggle. **Search results are `ModelResultCard`s** — name + MLX badge + repoID + a metadata line (**download size** (async via `resolveTotalSize` + bounded-concurrency `fetchSizes`) · download count · a **"Too big for your RAM"** chip via `ModelFit.fits`) + a **stateful action button** (`Download` → `Downloading N%` → `✓ Installed`). **GGUF handling (why a download "doesn't show up"):** a raw GGUF download is invisible to `ModelRegistry.scan` (MLX-only). `HFModel.isGGUF` / `isImageOrVideo` / `isConvertibleGGUF` classify each result: a **GGUF language model** shows a **"Download & convert"** button (`GGUFImportService.downloadAndConvert` → grabs just the Q8_0 → MLX; a top banner shows download/convert progress, the card shows "Converting…"); a **GGUF image/video** repo (FLUX/WAN) shows an amber **"GGUF"** info button → `ModelDetailSheet` (explains it can't run). `fetchSizes` skips GGUF (their size is the sum of every quant). Tapping any card opens `ModelDetailSheet`. **In-flight downloads are `DownloadProgressCard`s** ("1.2 GB of 5.4 GB" + brand bar). Rows use the shared `.card()` surface via `.cardRow()`. Per-local-row: trash + ✨ modify icon; context menu; **"Diffusion · chat + Code" badge** on `isDiffusion`; a **capability line** ("Chat · Fine-tune · Try it out · Code · Story · Practice", or "Chat · Code — not fine-tunable" for DiffusionGemma). Below the LLMs, an **"Image models (N)"** section (`ImageModelRow`, fed off-thread by `ImageGenService.downloadedImageModels()`) lists downloaded FLUX/SDXL/SD models (incl. single-file, badged) with their "Imagine · Story illustrations" capability + **disk size** (section Total too) — so every installed model is visible with what it supports, and image models don't silently vanish (they're not LLMs, so absent from `registry.localModels`). These rows are **managed like the LLM rows** (minus LLM-only actions): pin/favorite (`FavoritesStore`, pinned float to top), Rename, Show in Finder, and **Delete** (`ImageGenService.deleteImageModel`, off-thread, confirmation alert + shared "Freed X" result — a single-file checkpoint deletes just its `.safetensors`+blob+converted cache unless it's the last in the repo). `LowDiskWarningBanner` atop; total-disk header. |
| [`ModelDetailView.swift`](../LLMPro/Features/Models/ModelDetailView.swift) | **`ModelDetailSheet`** — a sheet shown when a HF result card is tapped (was an inline right-pane panel). Leads with the **size + RAM-fit verdict** ("≈ 4.29 GB · ✓ Runs on your Mac (128 GB)"), a humanized "Updated Sep 2024" metadata row, and **curated tags** (HF bookkeeping — `region:` / `base_model:` / `arxiv:` / `license:` — filtered out; license shown separately). Pulls `detail` + `resolveTotalSize` from HuggingFaceClient. CTAs: Download (primary), Teach (`.openTrainingWithModel`), Chat (`.openChatWithModel`). For non-mlx-community repos, offers a `mlx_lm.convert` step with quant picker. Keeps `FlowLayout`. |
...
A toolbar button **"Past lessons"** (clock.arrow.circlepath) opens
[`TrainingHistoryView.swift`](../LLMPro/Features/Monitor/TrainingHistoryView.swift)
as a sheet — a `@Query`'d list of every `TrainingJob` (most-recent first) with a
friendly status badge, date, and on-disk adapter size. Per-row trash + swipe
delete (disabled while a run is still training) and a "Delete all finished"
toolbar action. Deleting a run removes its SwiftData record, drops it from
`JobRegistry` (via `JobRegistry.remove(jobID:)`, which refuses a live process),
and deletes its `adapters/<uuid>/` folder — it does **not** touch the dataset,
base model, or any fused `…-trained` model produced from the run.
...
| [`ChatView.swift`](../LLMPro/Features/Chat/ChatView.swift) | `ChatPaneView` (one arena pane) + the shared **`MessageBubble`** (role glyph, markdown/code render via `MessageContentView`, copy + "Try again"). `MessageBubble` is now internal (was private) so the dedicated Chat tab reuses it. |

### Chat (`Features/Chat/` — the `.chatDirect` sidebar tab, distinct from "Try it out")

| File | Responsibility |
|---|---|
| [`ChatConversationView.swift`](../LLMPro/Features/Chat/ChatConversationView.swift) | The dedicated **Chat** tab: a clean single-model conversation with saved history. Left rail = conversation list (new / rename / delete, most-recent-first) driven by `ConversationStore`; right = model-picker menu + Persona menu (`SystemPromptPresetStore`) + temperature + Markdown export + clear, a transcript of `MessageBubble`s, and an input bar with Send/Stop (⌘↩ / ⌘.). Holds a live `session: ChatSession?` plus **`sessionConvID`** (the conversation the session was built from — persistence keys on this, NOT `selectedID`, so switching can't clobber the wrong conversation). `persist()` maps finalized turns back to the `StoredConversation` and auto-titles from the first user message; the outgoing session is `stop()`-ed before a new one loads. Any local model (incl. fine-tunes saved as `…-trained` models and DiffusionGemma) works — reuses the same `ChatSession`/`InferenceService` streaming as the arena. |
| [`ConversationStore.swift`](../LLMPro/Services/ConversationStore.swift) | `@MainActor @Observable`. Persists chat history as one `conversations/<uuid>.json` per conversation (`StoredConversation` = model + system prompt + temperature + `[StoredMessage]` + updatedAt). CRUD + `discardIfEmpty` (drops an unused "New chat"). No SwiftData. |

### Story (`Features/Story/` — the `.story` sidebar tab)

| File | Responsibility |
|---|---|
| [`StoryView.swift`](../LLMPro/Features/Story/StoryView.swift) | The **Story** tab: long-form chaptered creative writing. Left rail = project list (`StoryStore`); right = `StoryEditorPane` (`@Bindable generator`) — title + model picker + a settings disclosure (genre / creativity / words-per-chapter / target chapters / freeform style-&-tone / "Plan outline" / **illustrations-per-chapter (0–4) + art-style + image-model install gate**) + a premise editor + a scroll of chapter cards (each: streaming cursor, markdown body via `MessageContentView`, an inline **illustration gallery** (`LazyVGrid` of `IllustrationView` thumbnails, per-image Save/Show-in-Finder/Remove), and a menu: Revise… / Edit text… / Copy / **Illustrate·Redraw** / Delete) + a generation bar ("Write next chapter" with optional guidance, "Write to N" auto-loop, Stop, plus a live image-render progress line). Holds `generator: StoryGenerator?` + `generatorProjID` (persistence keys on it, not `selectedID`). `ReviseSheet` / `ChapterEditSheet` are the revise-instruction and manual-edit sheets; `IllustrationView` async-loads + caches each PNG off the main thread. |
| [`StoryGenerator.swift`](../LLMPro/Services/StoryGenerator.swift) | `@MainActor @Observable`. Owns the working `StoryProject` and streams chapters via `InferenceService`. **Long-story coherence**: each chapter's prompt = style/genre/premise + optional outline + a rolling set of per-chapter **summaries** (auto-generated after each chapter) + the previous chapter's tail — never the full prior text (a 15-chapter story exceeds any context window). `writeNextChapter(instruction:)`, `autoWriteToTarget()` (loop until `targetChapters` or Stop), `reviseChapter(id:instruction:)` (blank + re-stream; restores the original if empty), `planOutline()`, `summarize`, `stop()`. **Illustrations**: after a chapter is written (and on the manual `illustrateChapter(id:)`), `generateIllustrations` asks the LLM for N visual scenes (`extractScenes` → `parseScenes`), prepends the frozen `artStyle`, assigns deterministic seeds (`storyBaseSeed` from the project UUID + chapter offset + image index), and renders them in one `ImageGenService.generate` batch; runs inside the existing task (never bumps `generation`) and is a silent no-op if the image model isn't installed. Same `generation`-counter concurrency guard as `ChatSession`. Applies **no content policy** — latitude is the chosen model + the user's style text. |
| [`ImageGenService.swift`](../LLMPro/Services/ImageGenService.swift) | `@MainActor @Observable` singleton (Story illustrations + Imagine). `installed()`/`install()` gate the optional image add-on (`mflux` + Pillow/regex, enabling **both** engines; delegates to `PythonRuntime`). **Two engines, routed on `ImageModelFamily` (flux/sdxl/sd):** `generate([Request], family:model:baseModel:steps:cfg:negative:width:height:)` writes a batch JSONL and runs the family's helper **once** (`generate_image.py` for FLUX via mflux; `sdxl_generate.py` for SDXL/SD via the vendored MLX Stable Diffusion) via `ProcessRunner.spawn` inside a `withTaskCancellationHandler` (Stop terminates the subprocess); consumes the JSON-event stdout on the main actor, `progress` (done/total + `loadingModel`) drives the UI. FLUX loads from a repo id (mflux fetches it lazily); SDXL/SD load from a **local diffusers dir** resolved via `snapshotDir(for:)`, or a **single-file `.safetensors`** checkpoint (`ImageModel.checkpointFile`) converted once to a cached diffusers dir (`PathResolver.sdxlConvertedDir`, passed as `--convert-cache`). `ImageModel` carries `family`/`cfg`/`negative`/`checkpointFile` (id = `repo#file`); `SDXLVariant.detect` picks step/CFG/negative defaults by name (base/illustrious/pony/turbo/lightning/hyper). `downloadedNonPresetImageModels()` (`nonisolated static` — reads safetensors headers, so callers cache it) scans the HF cache: `classifyDiffusers()` tags diffusers models (model_index `_class_name` + `unet/`+`text_encoder_2/` markers) flux/sdxl/sd, and `singleFileCheckpoints()` finds single-file SDXL/SD checkpoints (header keys — skips repos with a `config.json`, i.e. LLMs). `downloadedImageModels()` adds downloaded presets for the Models-tab list; video checkpoints are skipped. **FLUX `defaultModel` is the ungated mirror `dhairyashil/FLUX.1-schnell-mflux-4bit`** with `defaultBaseModel = "schnell"`; passes `HF_TOKEN` (Keychain) when present. See [CONTRACTS §3 `generate_image.py` / `sdxl_generate.py`](CONTRACTS.md). |
| [`StoryStore.swift`](../LLMPro/Services/StoryStore.swift) | `@MainActor @Observable`. `StoryProject` (premise, style, genre, outline, model, temperature, chapterWordTarget, targetChapters, **illustrationsPerChapter, artStyle**, `[StoryChapter]`) + `StoryChapter` (title, text, summary, **`[StoryIllustration]` {prompt, file}**). Both have tolerant `init(from:)` decoders so files written before these fields load fine. Persists one `stories/<uuid>.json` per project; images live under `storyimages/<uuid>/` (removed on `delete`). CRUD + `discardIfEmpty`. No SwiftData. |
| [`StoryMarkdownExporter.swift`](../LLMPro/Features/Story/StoryMarkdownExporter.swift) | `@MainActor`. Renders a project (title + premise + all chapters, with `![](…)` links for each chapter's illustrations) to Markdown; on export copies the PNGs into a sibling `<stem>_images/` folder so the `.md` is self-contained. Saves via NSSavePanel. |

### Imagine (`Features/Imagine/` — the `.imagine` sidebar tab)

| File | Responsibility |
|---|---|
| [`ImagineView.swift`](../LLMPro/Features/Imagine/ImagineView.swift) | The **Imagine** tab: free-form local text-to-image. A prompt bar (prompt + a **model picker** grouped **On your Mac** vs **Download & use** — the built-in `ImageModel.presets` (FLUX.1 schnell/dev × 4-/8-bit) plus any **FLUX, SDXL, or SD** model already in the HF cache via `ImageGenService.downloadedNonPresetImageModels()` (diffusers **and single-file** `.safetensors` checkpoints — each single-file selectable separately, family-classified — chat LLMs and video models excluded; scanned once on appear into `@State scannedModels` off the main thread, since it reads headers) + a **size menu** (aspect Square/Landscape/Portrait/Wide/Tall × resolution Small/Standard/Large; FLUX → /16 long-edge, SDXL/SD → snapped to ~1 MP `sdxlBucket`) + count 1–4 + Generate/Stop + progress line) over a `LazyVGrid` gallery of results (`GalleryThumb`, async NSImage load). Builds N `ImageGenService.Request`s (random seeds) and calls `ImageGenService.generate(family:model:baseModel:steps:cfg:negative:width:height:)` — routing to the FLUX (mflux) or SDXL/SD (MLX Stable Diffusion) engine per the selected model, independent of the chat model. Model + aspect + resolution persist via `@AppStorage`. Cancellation-aware (Stop kills the subprocess + deletes the partial); an install gate offers the one-time image-engine install. Per-image: tap → `ImagePreviewSheet` (full-size + dims/seed + Save/Copy prompt/Delete); context menu adds "Use this prompt" / "Make another like this". |
| [`ImagineStore.swift`](../LLMPro/Services/ImagineStore.swift) | `@MainActor @Observable`. Persists the gallery as `imagegen/gallery.json` (`[GeneratedImage]` = prompt + file + seed + w/h + createdAt) alongside the PNGs in `imagegen/`. Same side-store pattern as `ConversationStore`/`StoryStore`; drops entries whose PNG went missing. `url(for:)` is `nonisolated` so the background image loader can use it. |

### Chat / Arena (`Features/Chat/`)
...
llama.cpp's converter for non-Llama/Mistral architectures. Each source row has a
context-menu + swipe **Delete** (with confirm alert) that removes the underlying
run — routed through the shared
[`TrainingArtifactDeletion`](../LLMPro/Services/TrainingArtifactDeletion.swift)
helper (same as Progress → "Past lessons").

`TrainingArtifactDeletion` (`@MainActor enum`, `Services/`) is the single delete
path for training artifacts: `deleteJob` removes a `TrainingJob` record +
`adapters/<uuid>/` (via `JobRegistry.remove`, refusing a live process);
`deleteRun` removes a `SelfImproveRun` record + `selfimprove/<uuid>/` (guarding the
in-flight generating/testing/training/evaluating statuses). Neither touches the
dataset, base model, or any fused `…-trained` model. Used by both
[`TrainingHistoryView`](../LLMPro/Features/Monitor/TrainingHistoryView.swift) and
[`ExportWizardView`](../LLMPro/Features/Export/ExportWizardView.swift).
...
| [`SettingsView.swift`](../LLMPro/Features/Settings/SettingsView.swift) | `TabView` with Runtime (venv rebuild [confirm-gated], keep-awake toggle, GGUF converter + compiled-tools installers) / Storage / Paths / Logs (live tail) / HuggingFace tabs. Shown in the standard macOS Settings scene. |
