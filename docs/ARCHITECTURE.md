...
| [`LLMPro/Services/ModelMetaStore.swift`](../LLMPro/Services/ModelMetaStore.swift) · [`Features/Models/ModelNotesSheet.swift`](../LLMPro/Features/Models/ModelNotesSheet.swift) | User notes + tags per local model (JSON keyed by model id, no schema change). Tag chips render in the model row; "Notes & tags…" context menu opens the sheet. |
