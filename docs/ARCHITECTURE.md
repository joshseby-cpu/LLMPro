...
| File | Responsibility |
|---|---|
| [`StoryView.swift`](../LLMPro/Features/Story/StoryView.swift) | The **Story** tab: long-form chaptered creative writing. Left rail = project list (`StoryStore`); right = `StoryEditorPane` (`@Bindable generator`) — title + model picker + a settings disclosure (genre / creativity / words-per-chapter / target chapters / freeform style-&-tone / "Plan outline") + a premise editor + a scroll of chapter cards (each: streaming cursor, markdown body via `MessageContentView`, and a menu: Revise… / Edit text… / Copy / Delete) + a generation bar ("Write next chapter" with optional guidance, "Write to N" auto-loop, Stop). Holds `generator: StoryGenerator?` + `generatorProjID` (persistence keys on it, not `selectedID`). `ReviseSheet` / `ChapterEditSheet` are the revise-instruction and manual-edit sheets. |
| [`StoryGenerator.swift`](../LLMPro/Services/StoryGenerator.swift) | `@MainActor @Observable`. Owns the working `StoryProject` and streams chapters via `InferenceService`. **Long-story coherence**: each chapter's prompt = style/genre/premise + optional outline + a rolling set of per-chapter **summaries** (auto-generated after each chapter) + the previous chapter's tail — never the full prior text (a 15-chapter story exceeds any context window). `writeNextChapter(instruction:)`, `autoWriteToTarget()` (loop until `targetChapters` or Stop), `reviseChapter(id:instruction:)` (blank + re-stream; restores the original if empty), `planOutline()`, `summarize`, `stop()`. Same `generation`-counter concurrency guard as `ChatSession`. Applies **no content policy** — latitude is the chosen model + the user's style text. |
| [`StoryStore.swift`](../LLMPro/Services/StoryStore.swift) | `@MainActor @Observable`. `StoryProject` (premise, style, genre, outline, model, temperature, chapterWordTarget, targetChapters, `[StoryChapter]`) + `StoryChapter` (title, text, summary). Persists one `stories/<uuid>.json` per project; CRUD + `discardIfEmpty`. No SwiftData. |
| [`StoryMarkdownExporter.swift`](../LLMPro/Features/Story/StoryMarkdownExporter.swift) | `@MainActor`. Renders a project (title + premise + all chapters) to Markdown, saves via NSSavePanel. |
