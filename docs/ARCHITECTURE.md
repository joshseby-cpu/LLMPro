...
| File | Responsibility |
|---|---|
| [`ImagineView.swift`](../LLMPro/Features/Imagine/ImagineView.swift) | The **Imagine** tab: free-form local text-to-image. A prompt bar (prompt + a size menu Square/Portrait/Landscape + a count 1–4 + Generate/Stop + progress line) over a `LazyVGrid` gallery of results (`GalleryThumb`, async NSImage load). Builds N `ImageGenService.Request`s (each a random seed) and calls `ImageGenService.generate(...)` — the **same FLUX engine as Story illustrations**, so it works regardless of the chat model. Cancellation-aware (Stop kills FLUX + deletes the partial). An install gate offers the one-time `mflux` install when absent. Per-image: tap → `ImagePreviewSheet` (full-size + dims/seed + Save/Copy prompt/Delete); context menu adds "Use this prompt" / "Make another like this". |
