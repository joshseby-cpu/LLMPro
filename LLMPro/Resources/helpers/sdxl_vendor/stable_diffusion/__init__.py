...
class StableDiffusionXL(StableDiffusion):
    ...
    def generate_latents(
        ...
        # LLMPro: SDXL add_time_ids = (orig_h, orig_w, crop_top, crop_left,
        # target_h, target_w). Upstream hardcodes 512; derive from the real latent
        # size (×8) so 1024² (and other buckets) get correct micro-conditioning —
        # otherwise composition/framing degrades badly at non-512 sizes.
        img_h, img_w = latent_size[0] * 8, latent_size[1] * 8
        text_time = (
            pooled_conditioning,
            mx.array([[img_h, img_w, 0, 0, img_h, img_w]] * len(pooled_conditioning)),
        )
