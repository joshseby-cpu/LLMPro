...
LLMPro prepends this to EVERY mlx_lm invocation — training, inference, AND the
...
Env (all optional):
    LLMPRO_MEMORY_LIMIT_BYTES   override the soft memory limit (Memory tab cap)
    LLMPRO_CACHE_LIMIT_BYTES    override the buffer-cache limit
    LLMPRO_NO_AUTOTUNE=1        skip all auto-tuning (use stock MLX defaults)

Usage (as prepended by LLMPro):
...
def _apply_limits() -> None:
    if os.environ.get("LLMPRO_NO_AUTOTUNE") == "1":
        return
    try:
    ...
    mem_limit = None
    raw = os.environ.get("LLMPRO_MEMORY_LIMIT_BYTES")
    if raw:
        try:
            mem_limit = int(raw)
        except ValueError:
    ...
    craw = os.environ.get("LLMPRO_CACHE_LIMIT_BYTES")
