...
/// app vanished" into "the app vanished AND left a stack trace in llmpro.log".
...
/// Central logging for LLMPro. Every message fans out to TWO sinks:
...
///  2. **A persistent file** at `PathResolver.logsDir/llmpro.log` — survives
...
enum Log {
    ...
    private static let subsystem = Bundle.main.bundleIdentifier ?? "com.josh.llmpro.LLMPro"
    ...
    static func install() {
        ...
        write(.notice, .app, "──────── LLMPro launch · v\(v) (\(b)) · \(os) ────────",
    ...
    private static func installCrashHandlers() {
        ...
        crashMarker = Array("\n\n=== LLMPro CRASHED (fatal signal) — backtrace follows ===\n".utf8)
...
private final class LogFileSink: @unchecked Sendable {
    static let url = PathResolver.logsDir.appendingPathComponent("llmpro.log")
    private static let rotatedURL = PathResolver.logsDir.appendingPathComponent("llmpro.log.1")
    private static let maxBytes = 5 * 1024 * 1024   // 5 MB before rotation

    private let queue = DispatchQueue(label: "com.llmpro.log.file", qos: .utility)
