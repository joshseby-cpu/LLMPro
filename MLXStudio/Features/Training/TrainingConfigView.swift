...
struct TrainingConfigView: View {
    ...
    private var headline: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("Teach a model to code, in three steps.")
                .font(.title2.bold())
            Text("MLX Studio will pick the best settings for your Mac. You don't need to know how training works.")
                .font(.callout)
    ...
    private var step3DurationPicker: some View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let preview {
                Text("MLX Studio will train for **\(preview.iters) steps**, in batches of **\(preview.batchSize)**. It should take **about \(preview.estimatedMinutes) minute\(preview.estimatedMinutes == 1 ? "" : "s")**.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
...
extension Notification.Name {
    static let switchToMonitor = Notification.Name("MLXStudio.switchToMonitor")
    ...
    static let openTrainingForCoding = Notification.Name("MLXStudio.openTrainingForCoding")
}
