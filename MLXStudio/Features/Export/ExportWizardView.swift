            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    if target == .fusedSafetensors {
                        Text("Will produce safetensors in `~/Library/Application Support/MLXStudio/exports/<id>-fused/`.").font(.callout)
                        Toggle("Also install in LM Studio (~/.lmstudio/models/MLXStudio/<name>/)",
                               isOn: $alsoInstallInLMStudio)
                        if alsoInstallInLMStudio {
                            TextField("LM Studio model name", text: $lmstudioFusedName)
                                .textFieldStyle(.roundedBorder)
                                .help("Lands under ~/.lmstudio/models/MLXStudio/<this name>/")
                        }
                    }
                    if target == .adapter {
            ...
            case .fusedSafetensors:
                ...
                if alsoInstallInLMStudio {
                    ...
                    if p.terminationStatus == 0 {
                        progress("✅ Installed in LM Studio: ~/.lmstudio/models/MLXStudio/\(name)/")
                    } else {
